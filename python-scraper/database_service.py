...
class DatabaseService:
    ...
    async def save_office(self, office: PermitOffice) -> Optional[str]:
        """Save a permit office to the database"""
        try:
    ...
    async def save_offices_batch(self, offices: List[PermitOffice]) -> Dict[str, Any]:
        """Save multiple offices in a batch"""
        results = {
            'saved': 0,
            'updated': 0,
            'errors': 0,
        ...
        for office in offices:
            try:
                office_id = await self.save_office(office)
                if office_id:
                    results['office_ids'].append(office_id)
                    if office.id:
                        results['updated'] += 1
                    else:
                        results['saved'] += 1
                ...
                logger.error(f"Error saving office {office.department_name}: {e}")
                results['errors'] += 1
